...
method_list = [{'DecisionTree()':tree.DecisionTreeClassifier(),
                ...
                'svm.SVC(C=1)':svm.SVC(),
                'svm.SVC(C=1.5)':svm.SVC(C=1.5),
                'svm.SVC(C=2)':svm.SVC(C=2),
                'svm.SVC(kernel=linear, C=2)':svm.SVC(kernel='linear'),
                'svm.SVC(kernel=poly, C=2)':svm.SVC(kernel='poly'),
                'RandomForest(random_state=10)':RandomForestClassifier(random_state=10), 
                 'XGBT()':Gxb(),
                # 'XGBT(Lambda=0.8)':Gxb(reg_lambda=0.8),
                'XGBT(Lambda=1.2)':Gxb(reg_lambda=1.2),
                'XGBT(Lambda=1.4)':Gxb(reg_lambda=1.4),
                'XGBT(gamma=0.2)': Gxb(gamma=0.2),
                'XGBT(gamma=0.3)': Gxb(gamma=0.3),
                'XGBT(gamma=0.4)': Gxb(gamma=0.4),
                'XGBT(gamma=0.5)': Gxb(gamma=0.5),
                'XGBT(gamma=0.55)': Gxb(gamma=0.55)
                }]#
