...
#Load ticker data
stocklist=['MSFT','AAPL','AMZN','GOOG','FB','JNJ','V','PG','JPM','UNH','MA','INTC','VZ','HD','T',
           'PFE','MRK','PEP']
for stock in stocklist:
    ...
    df.loc[(df['K']>df['D']) & (df['K_prev']<df['D_prev']) & (df['D']<=10) ,'Intersection'] = 1# Intersections: K go exceeds D
    ...
    for method in method_list.loc[0,:]:
        ...
        series={'Stock':stock,'Method':method_list.columns[index],'AvgScores':scores.mean(),'StdScores':scores.std()}
        index=index+1
